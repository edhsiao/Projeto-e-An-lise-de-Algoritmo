    ...
    while(len <= dim)
    {
        for( i = 0, j = len - 1 + i; j < dim; i++, j++)
        {
            ...
            else
            {
                ...
                else // seq_dna[i] == seq_dna[j]
                {
                    if((vetorM[i] == 1) && (vetorM[j] == 1))//ambos mutante
                    {
                    ...
                    else
                    {
                        ...
                        else //nao tem mutacao
                        {
                            if(matriz[i][j - 1].mutacao == matriz[i + 1][j].mutacao)
                            {
                            matriz[i][j].mutacao = max(matriz[i][j - 1].mutacao,matriz[i + 1][j].mutacao);
                              matriz[i][j].maior_seq = max(matriz[i][j - 1].maior_seq,matriz[i + 1][j].maior_seq) + 2;
                            }
                            else if (matriz[i][j - 1].mutacao > matriz[i + 1][j].mutacao)
                            {
                                matriz[i][j].mutacao = matriz[i][j - 1].mutacao;
